...
Condivisione peer-to-peer offline di una carta fedeltà tramite un QR code self-contained: un device genera un QR che incapsula lo snapshot completo della carta (payload compatto, versionato, con magic prefix), e un altro device lo scansiona per crearne una copia indipendente. Nessun server, nessun legame persistente tra i device. La ricezione (scansione + conferma) è coperta da `card-capture`.
