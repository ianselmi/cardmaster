...
- **Le righe di uno scontrino già salvato si possono ricavare a posteriori dal `RawText`?** Tecnicamente sì. Se offrirlo, e con quale interfaccia, si decide alla verifica su scontrini reali: ha senso solo se la ricostruzione funziona abbastanza da non produrre lavoro di correzione peggiore di rifare la foto.
- **Quante categorie deve avere il seed?** Poche e larghe (dell'ordine della decina) sono utili subito e reggono le viste di `receipt-insights`; molte e fini classificano meglio ma sbagliano di più e rendono le classifiche illeggibili. Si parte larghi; l'apprendimento locale può sempre specializzare.
