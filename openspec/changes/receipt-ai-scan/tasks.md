...
- [x] 3.4 Modello selezionabile con `claude-opus-5` come default; nessun altro identificativo di modello inventato o costruito a mano
      ...
      → **Default messo alla prova** (13 ago 2026, stesso scontrino e stessa immagine): Opus 5 fa **29 righe su 29** e quadra a 47,74 € per 12,064 ¢; **Haiku 4.5 non quadra**, con uno scarto di almeno 6,60 € su 47,74, per 1,322 ¢. Costa un decimo e non fa il lavoro. Il default resta Opus 5 e la misura è registrata in `design.md`; Haiku resta selezionabile per chi preferisce spendere meno sapendo cosa rischia. `claude-sonnet-5` non è stato misurato: è lì che guardare per primo se un giorno il default dovesse cambiare per costo.
      → Nel fallimento di Haiku si è visto funzionare dal vivo il 4.6: la rilettura è arrivata, il confronto l'ha bocciata, e **le righe buone non sono state toccate**.
...
- [x] 5.2 Costo indicativo per scontrino accanto a ogni modello, e consumo effettivo dell'ultima chiamata
      → Ogni voce del selettore porta il costo accanto. La stima viene da `ReceiptAiModels.TypicalUsage`, **la stessa** usata nel messaggio di consenso prima dell'invio: due fonti diverse sarebbero divergute. Sotto, il costo effettivo dell'ultima rilettura con i token reali — che è il controllo della stima, **e che ha subito trovato un errore da 3×** (vedi 7.3).
      → **Difetto trovato e corretto in 7.3**: la riga del costo effettivo non si aggiornava nella sessione in cui la rilettura avveniva. Le Impostazioni restano vive sullo stack della loro tab, quindi tornandoci dopo una rilettura fatta sull'altra tab non si passa da `OnAppearing` e si vedeva il valore di prima. Ora `ISettingsStore` espone `AiUsageChanged` e il ViewModel si aggiorna quando il dato cambia, invece di dipendere dal ciclo di vita della pagina. La sottoscrizione usa `WeakEventManager`: lo store è singleton e i ViewModel transienti, quindi un handler forte li terrebbe in vita per tutta la sessione.
...
- [x] 7.2 Verifica su emulatore con lo **stesso scontrino MD** di `receipt-items` (21 righe su 29 in locale): riportare quante righe su quante produce la rilettura e se la somma quadra — un numero, non un giudizio
      → **29 righe su 29. La somma quadra a tolleranza zero: 47,74 €.** (Emulatore API 36, chiave reale, 13 ago 2026.) Ogni importo verificato uno per uno contro la foto dello scontrino.
      Oltre alle righe, la rilettura ha corretto **il totale**: `41,14` → `47,74`. È l'errore previsto in `design.md` — l'OCR aveva letto male una cifra del totale stampato, e solo l'immagine poteva correggerlo. Ha anche riempito la **partita IVA**, che era vuota (`03185210618`), lasciando invece intatto l'esercente già presente.
      Le righe appaiate sono separate: `*PRIMOSALE S/LATTOSIO V M4,00 *MORTADELLA BOL.OGNA IGP` era **una** riga con aliquota 10%, ed è tornata a essere due righe — al **4%** e al **10%**, cioè con l'aliquota giusta ciascuna. Riconosciute anche la riga a peso (`Zucchine scure`, 0,546 kg) e i due sconti, negativi.
- [x] 7.3 Verifica del costo reale di quella chiamata, confrontato con la stima dichiarata nelle impostazioni
      → **Misurato: 12,064 ¢** (3.578 token in ingresso, 4.110 in uscita) contro una stima dichiarata di **4 ¢**. La stima era **tre volte sotto**, e sbagliava soprattutto sull'uscita: un JSON di 29 righe con descrizione, quantità, unità, prezzo unitario, importo e aliquota costa molto più di quanto sembri contando le righe.
      **Corretto**: `ReceiptAiModels.TypicalUsage` ora usa i valori misurati e il selettore dichiara «circa 12,05 ¢ a scontrino». Dichiarare 4 centesimi e addebitarne 12 avrebbe tradito la promessa di rendere il costo visibile *prima* — che è metà del senso di questa change. Rimisurare su altri scontrini prima di considerare la stima assestata: 29 righe sono un caso grande, non medio.
